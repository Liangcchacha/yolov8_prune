...
def prune_conv(conv1: Conv, conv2: Conv):
    gamma = conv1.bn.weight.data.detach()  #取出gamma和beta
    beta  = conv1.bn.bias.data.detach()
    
    keep_idxs = []  #需要保留的channel idxs
    local_threshold = threshold
    while len(keep_idxs) < 8:  # 如果裁剪的通道数小于8，就是裁剪太多了，把阈值降低
...
seq = model.model
for i in range(3, 9):
    if i in [6, 4, 9]: continue  # 这个部分跳过不剪枝，因为fpn结构里面又很多concat前面层的操作，比较麻烦
    prune(seq[i], seq[i+1])
